...
|---|---|---|
| 09:09:23 | outage bắt đầu — region-a bị netblock (SIGSTOP), `other_alive:true` | `chaos/chaos-events.jsonl:3` |
| 09:09:23 | user đầu tiên bị ảnh hưởng — ReadTimeout 2262.9ms qua edge | `reports/drill-2-withdr.jsonl:25` |
| 09:09:38 | health check alert — region-a `UNHEALTHY` sau 3 lần fail liên tiếp | `reports/health-events.jsonl:2` |
| 09:09:39 | operator confirm + mở incident, bấm giờ RTO (`notify_lag_s:15.34`) | `reports/runbook-run.jsonl:2` |
| 09:09:39 | snapshot restore xong (`rpo_seconds:6.0`, `docs_lost:3`) | `reports/failover-events.jsonl:2` |
| 09:09:45 | region-b ready sau warm-up 6.36s, DNS cutover a→b | `reports/failover-events.jsonl:5` |
| 09:09:48 | resolved — request đầu tiên OK, `served_by:b` | `reports/drill-2-withdr.jsonl:36` |

## 2. RTO/RPO đo được vs mục tiêu — gap ở bước nào?

- RTO mục tiêu: 300s · đo được: **24.9s** · gap: **−275.1s** (dưới mục tiêu, PASS)
- RPO mục tiêu: 300s · đo được: **6.0s** (**3** doc bị mất) · gap: **−294.0s** (PASS)
- **Bước tốn nhiều giây nhất:** `health-check detect floor` — **14.9s / 24.9s = 59.8%**.
...
phía sau (restore 0.7s + warm-up 6.3s + TTL 3.0s = 10.0s) rẻ hơn riêng cái sàn này.

Đáng chú ý: gap thật của drill này **không nằm ở RTO**. RTO 24.9s dư 275s so với mục
tiêu. Chỗ đáng lo là RPO — 6.0s nghe rất nhỏ chỉ vì `replicate.py --every 30` chạy dày
...
1. **Vì sao user mất 24.9s?** Vì traffic vẫn trỏ vào region-a suốt 21.9s đầu.
...
|---|---|---|---|---|
| 1 | Giữ region-b ở `pool_state=full` (warm standby) thay vì `warm`, bỏ hẳn bước scale trong đường failover | Infra on-call | 2026-09-08 | −6.3s RTO (bỏ toàn bộ warm-up) |
...
| 4 | Chuyển vector DB sang continuous replication (WAL ship) thay vì snapshot chu kỳ | Data platform | 2026-10-01 | RPO 6.0s → < 1s; ở lịch thật 6h → phút |
...
5s × 3 = 15.0s (`detect_floor_s` ghi ngay trong `reports/health-events.jsonl:2`). Thực
đo 14.9s vì cú kill rơi vào giữa hai lần poll. 14.9 / 24.9 = **59.8% RTO**. Đây là thành
phần lớn nhất, lớn hơn tổng của cả ba thành phần còn lại.

**2. Nếu hạ interval xuống 1s, RTO giảm mấy giây — và bạn trả giá gì?**
Floor còn 1 × 3 = 3s, RTO ước tính còn ~13s (giảm ~12s). Giá phải trả: cửa sổ bằng
...
`docs_lost:3` (`reports/failover-events.jsonl:2`) không phải "3 dòng trong SQLite". Đó
là 3 ticket khách hàng đã gửi, hệ thống đã trả lời "đã nhận", và bây giờ không tồn tại
ở đâu nữa. Không ai báo cho khách biết, vì chính chúng ta cũng không biết đó là ticket
nào — RPO tính bằng giây không cho biết *ai* mất gì. Với 3 doc, việc phải làm là dump
