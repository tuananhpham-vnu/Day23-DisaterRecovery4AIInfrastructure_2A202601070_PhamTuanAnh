...
edge vượt 50% quá 15s. **Mục tiêu:** RTO ≤ 300s. **Đo được lần gần nhất: 24.9s.**
...
| 6 | Verify golden signals | `for i in $(seq 10); do curl -s -o /dev/null -w '%{http_code} %{time_total}\n' localhost:8080/v1/infer; done` | 10/10 trả `200`, p95 < 1000ms, error rate = 0. Lần chạy gần nhất: p95 391.5ms, error rate 0.0 (`reports/runbook-run.jsonl:6`) | on-call secondary |
...
lần lật thêm là thêm ~25s downtime chứ không phải bớt (§4 Anti-Patterns).
